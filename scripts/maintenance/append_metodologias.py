...
async function showMethodologyResults(lineName) {
    ...
    styleBlock.innerHTML = `
        ...
        .res-card-header { padding: 16px 20px; font-weight: 600; font-size: 1.1rem; border-bottom: 1px solid #e5e7eb; display: flex; align-items: center; gap: 8px; background-color: #f9fafb; }
    ...
    function renderAsia() {
        ...
        let factor = (r * Math.pow(1 + r, n)) / (Math.pow(1 + r, n) - 1);
        let annuity = capex * (factor || 0);
        ...
        contentContainer.innerHTML = `
            <div class="res-card">
                <div class="res-card-header">Configuración Financiera (ASIA)</div>
                <div class="res-card-body" style="background:#f9fafb;">
                    <div style="display: flex; gap: 10px; flex-wrap: wrap; margin-bottom: 12px;">
                        <input type="number" id="asia_capex" class="form-input" style="width:140px;" value="${asiaInputs.capex_usd}" placeholder="CAPEX (USD)">
                        <input type="number" id="asia_n" class="form-input" style="width:100px;" value="${asiaInputs.life_years}" placeholder="Años">
                        <input type="number" id="asia_r" class="form-input" style="width:100px;" value="${asiaInputs.r_annual}" step="0.01" placeholder="r Anual">
                        <input type="number" id="asia_om" class="form-input" style="width:100px;" value="${asiaInputs.om_frac_annual}" step="0.01" placeholder="OM (0.01)">
                        <input type="number" id="asia_miles" class="form-input" style="width:100px;" value="${asiaMilesLine}" placeholder="Millas">
                        
                        <select id="asia_rule" class="form-input" style="width:auto;">
                            <option value="importer_pays" ${asiaMwMileRule==='importer_pays'?'selected':''}>MW-Mile: Importador Paga</option>
                            <option value="exporter_pays" ${asiaMwMileRule==='exporter_pays'?'selected':''}>MW-Mile: Exportador Paga</option>
                            <option value="split_50_50" ${asiaMwMileRule==='split_50_50'?'selected':''}>MW-Mile: 50% / 50%</option>
                        </select>
                        <select id="asia_method" class="form-input" style="width:auto;">
                        ...
                        <label style="display:flex; align-items:center; gap:5px; font-size:13px; font-weight:bold;">
                            <input type="checkbox" id="asia_ptoyear" ${asiaProjectToYear?'checked':''}> Proyectar a 1 Año
                        </label>
                        <button id="asia_recalc" style="padding: 6px 12px; background:#2563eb; color:white; border:none; border-radius:4px; cursor:pointer;">Recalcular</button>
                    </div>
                </div>
            </div>

            <div style="display: flex; gap: 20px;">
                <div class="res-card" style="flex: 1;">
                    ...
                    <div class="res-card-body">
                        <table class="res-table">
                            ...
                            <tr style="background:#e0e7ff;"><td style="color:#1e40af">RR Objetivo de Cálculo:</td><td align="right" style="font-weight:bold; color:#1e40af">${formatCur(target_rr)} ${asiaProjectToYear?'/año':'/hora'}</td></tr>
                            ...
                            <tr><td>Tarifa MW-Mile Unitaria:</td><td align="right">${formatCur(mwm_unit)}</td></tr>
                ...
                <div class="res-card" style="flex: 2;">
                    <div class="res-card-header">Resultados Ex-Post (Método Activo: ${asiaExpostMethod.toUpperCase()})</div>
                    <div class="res-card-body">
                        <table class="res-table">
                            <thead><tr><th>Métrica Ex-Post ($)</th><th>${expC} (Exp)</th><th>${impC} (Imp)</th></tr></thead>
                            <tbody>
                                <tr>
                                    <td>${createTooltipHTML('1. Costo Operación', 'Costo Op + Peaje')} (CON)</td>
                                    <td>${formatCur(conData.bsE)}</td><td>${formatCur(conData.bsI)}</td>
                                </tr>
                                <tr style="background-color:#f0fdf4">
                                    <td>Δ (CON - SIN) [Ganador si < 0]</td>
                                    <td>${formatCur(deltaBsE)}</td><td>${formatCur(deltaBsI)}</td>
                                </tr>
                                <tr>
                                    <td>${createTooltipHTML('2. Beneficio Generador', 'Profit + Rent - Peaje')} (CON)</td>
                                    <td>${formatCur(conData.bgE)}</td><td>${formatCur(conData.bgI)}</td>
                                </tr>
                                <tr style="background-color:#f0fdf4">
                                    <td>Δ (CON - SIN) [Ganador si > 0]</td>
                                    <td>${formatCur(deltaBgE)}</td><td>${formatCur(deltaBgI)}</td>
                                </tr>
                                <tr>
                                    <td>${createTooltipHTML('3. Costo Demanda', 'Costo Dem + Peaje')} (CON)</td>
                                    <td>${formatCur(conData.cdE)}</td><td>${formatCur(conData.cdI)}</td>
                                </tr>
                                <tr style="background-color:#f0fdf4">
                                    <td>Δ (CON - SIN) [Ganador si < 0]</td>
                                    <td>${formatCur(deltaCdE)}</td><td>${formatCur(deltaCdI)}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
    ...
    function renderEuropa() {
        ...
        contentContainer.innerHTML = `
            <div class="res-card">
                <div class="res-card-header">Configuración ENTSO-E (EUROPA)</div>
                <div class="res-card-body" style="background:#f9fafb;">
                    <div style="display: flex; gap: 10px; flex-wrap: wrap; margin-bottom: 12px;">
                        <input type="number" id="eur_itc" class="form-input" style="width:120px;" value="${europaInputs.itc_rate_usd_per_mwh}" step="0.01" placeholder="Rate ITC">
                        <input type="number" id="eur_cid" class="form-input" style="width:120px;" value="${europaInputs.cid_split}" step="0.01" placeholder="CID Split (0 a 1)">
                        <input type="number" id="eur_fund" class="form-input" style="width:140px;" value="${europaInputs.infra_fund_usd_per_hour}" placeholder="Fondo Infra ($/h)">
                        <select id="eur_opt" class="form-input" style="width:auto;">
                            <option value="A1" ${europaExpostMethod==='A1'?'selected':''}>Ex-Post Opción A1 (ITC Pay Separado)</option>
                            <option value="A2" ${europaExpostMethod==='A2'?'selected':''}>Ex-Post Opción A2 (Peaje Neto (ITC-CID))</option>
                        </select>
                        <button id="eur_recalc" style="padding: 6px 12px; background:#10b981; color:white; border:none; border-radius:4px; cursor:pointer;">Recalcular</button>
                    </div>
                </div>
            </div>
            ...
            <div class="res-card">
                ...
                <div class="res-card-body">
                    <table class="res-table text-right">
                        <thead><tr>
                            <th class="text-left">TSO País</th><th>Flujo Neto (NF)</th><th>Pago ITC ($)</th><th>Ingreso CID ($)</th><th>Ingreso Infra ($)</th><th>Liquidación Final</th>
                        </tr></thead>
                        <tbody>
                            <tr>
                                <td class="text-left">${expC} (Exp)</td><td>${formatNum(nf_exp)}</td>
                                ...
                                <td style="font-weight:bold; color:#15803d">${formatCur(set_e)}</td>
                            </tr>
                            <tr>
                                <td class="text-left">${impC} (Imp)</td><td>${formatNum(nf_imp)}</td>
                                ...
                                <td style="font-weight:bold; color:#15803d">${formatCur(set_i)}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            ...
            <div class="res-card">
                ...
                <div class="res-card-body">
                    <table class="res-table">
                        <thead><tr><th>Agente / Métrica</th><th>${expC}</th><th>${impC}</th></tr></thead>
                        <tbody>
    ...
    function renderSiepac() {
        ...
        let uso = Math.min(Math.abs(F) / C, 1.0);
        ...
        let curtrc = MR / Rc; let curtrg = MI / Ig;

        let pRet = curtrc * Math.abs(F); let pIny = curtrg * Math.abs(F);
        ...
        contentContainer.innerHTML = `
            <div class="res-card">
                <div class="res-card-header">Configuración Semestral (SIEPAC)</div>
                <div class="res-card-body" style="background:#f9fafb;">
                    <div style="display: flex; gap: 10px; flex-wrap: wrap; margin-bottom: 12px;">
                        <input type="number" id="sie_capex" class="form-input" style="width:140px;" value="${siepacInputs.capex}" placeholder="CAPEX (USD)">
                        <input type="number" id="sie_alphaR" class="form-input" style="width:100px;" value="${siepacInputs.alpha_R}" step="0.01" placeholder="Alpha R">
                        <input type="number" id="sie_alphaI" class="form-input" style="width:100px;" value="${siepacInputs.alpha_I}" step="0.01" placeholder="Alpha I">
                        
                        <select id="sie_country" class="form-input" style="width:auto;">
                        ...
                        <button id="sie_recalc" style="padding: 6px 12px; background:#eab308; color:white; border:none; border-radius:4px; cursor:pointer;">Recalcular</button>
                    </div>
                </div>
            </div>
            ...
            <div style="display: flex; gap: 20px;">
                <div class="res-card" style="flex:1;">
                    <div class="res-card-header">Remuneración SIEPAC</div>
                    <div class="res-card-body">
                        <table class="res-table">
                            <tr><td>Ingreso Req_Semestral (IR):</td><td align="right" style="font-weight:bold; color:#b45309">${formatCur(ir)}</td></tr>
                            <tr><td>Factor de Uso:</td><td align="right">${(uso*100).toFixed(2)}%</td></tr>
                            <tr><td>CARGO POR USO (P):</td><td align="right">${formatCur(P)}</td></tr>
                            <tr><td>CARGO COMPL. (CC):</td><td align="right">${formatCur(CC)}</td></tr>
                            <tr><td>CURTR_C (Retiros Demanda):</td><td align="right">${formatCur(curtrc)} /MWh</td></tr>
                            <tr><td>CURTR_G (Inyecciones Gen):</td><td align="right">${formatCur(curtrg)} /MWh</td></tr>
                        </table>
                    </div>
                </div>

                <div class="res-card" style="flex:1;">
                    <div class="res-card-header">Ex-Post (${siepacTargetCountry}) Semestral</div>
                    <div class="res-card-body">
                        <table class="res-table">
                            <thead><tr><th>Métrica</th><th>Valor Semestral</th></tr></thead>
                            <tbody>
                                <tr><td>Costo Operación Obj</td><td align="right">${formatCur(bsT)}</td></tr>
                                <tr><td>Beneficio Generador Obj</td><td align="right">${formatCur(bgT)}</td></tr>
                                <tr><td>Costo Demanda Obj</td><td align="right">${formatCur(cdT)}</td></tr>
                            </tbody>
                        </table>
                    </div>
                </div>
...
target_file = r"public\chart_helper.js"

with open(target_file, "r", encoding="utf-8") as f:
    lines = f.readlines()

# find index of the old function start
start_idx = -1
for i, line in enumerate(lines):
    if "async function showMethodologyResults" in line:
        start_idx = i
        break
    
if start_idx != -1:
    # Also drop the comment above it if present
    if start_idx > 0 and "// Function to load and display" in lines[start_idx-1]:
        start_idx -= 1
...
print("Updated chart_helper.js with the new tabbed Methodology UI.")
